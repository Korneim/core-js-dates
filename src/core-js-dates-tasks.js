...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (new Date(dateEnd) - new Date(dateStart)) / 1000 / 60 / 60 / 24 + 1;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= daysInMonth; i += 1) {
    ...
    const strDay = new Intl.DateTimeFormat('en-US', options).format(date);

    if (strDay === 'Sunday' || strDay === 'Saturday') {
      count += 1;
    }
  }
...
function getNextFridayThe13th(date) {
  ...
  while (itTrue) {
    ...
    for (let j = day; j <= daysInMonth; j += 1) {
      const currentDate = new Date(year, month, j);
      const strDay = new Intl.DateTimeFormat('en-US', options).format(
        currentDate
      );
